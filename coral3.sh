...
EOF
    ...
    msg_info "Instalando controladores NVIDIA en el contenedor ${CONTAINER_ID}"
    pct exec "$CONTAINER_ID" -- bash -c "apt-get update && apt-get install -y nvidia-utils" &>/dev/null
    msg_ok "Configuración de NVIDIA completada en el contenedor ${CONTAINER_ID}."
}
...
EOF
    ...
    msg_info "Instalando drivers de Coral TPU en el contenedor ${CONTAINER_ID}"
    pct exec "$CONTAINER_ID" -- bash -c "apt-get update && apt-get install -y libedgetpu1-max" &>/dev/null
    msg_ok "Configuración de Coral TPU completada en el contenedor ${CONTAINER_ID}."
}

# Configurar LXC para combinaciones iGPU + Coral TPU
configure_lxc_for_igpu_and_coral() {
    configure_lxc_for_igpu
    configure_lxc_for_coral
}

# Configurar LXC para combinaciones NVIDIA + Coral TPU
configure_lxc_for_nvidia_and_coral() {
    configure_lxc_for_nvidia
    configure_lxc_for_coral
}
...
main_menu() {
    ...
    OPTIONS=(
        ...
        "2" "Añadir NVIDIA"
        "3" "Añadir Coral TPU"
        "4" "Añadir iGPU + Coral TPU"
        "5" "Añadir NVIDIA + Coral TPU"
    )

    OPTION=$(whiptail --title "Menú Principal" --menu "Selecciona una opción:" 15 60 6 "${OPTIONS[@]}" 3>&1 1>&2 2>&3)

    case "$OPTION" in
        1)
            select_container
            configure_lxc_for_igpu
            msg_ok "Configuración de iGPU completada."
            ;;
        2)
            select_container
            configure_lxc_for_nvidia
